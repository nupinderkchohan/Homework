...
def get_genre():
    ...
    while True:
        ...
        if x.isdigit() and 1 <= int(x) <= len(genres): return[int(x) - 1]
        x = x.title()
...
def get_rating():
    while True:
        ...
        try:
            ...
            print(Fore.RED + "Invalip input. Try again.\n")
...
while True:
    a = input(Fore.YELLOW + "\nWould you like more recmmendations? (yes/no): ").strip().lower()
    if a == "no":
